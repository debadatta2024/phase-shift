                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {!isLoggedIn ? (
                          ...
                          <>
                            ...
                            <button onClick={() => navigateTo('profile')} className="text-slate-600 hover:text-teal-600 transition-colors font-medium">Profile</button>
                            <button onClick={onLogout} className="bg-rose-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-rose-600 transition-all">
                                Logout
                            </button>
                          </>
                        )}
            ...
            {isMenuOpen && (
                <div className="md:hidden bg-white/80 backdrop-blur-lg pb-4">
                     <nav className="flex flex-col items-center space-y-4 pt-2">
                        {!isLoggedIn ? (
                          <>
                            <a href="#simplifier" onClick={() => { setIsMenuOpen(false); navigateTo('home'); }} className="text-slate-600 hover:text-teal-600 transition-colors font-medium">Simplifier</a>
                            <a href="#how-it-works" onClick={() => { setIsMenuOpen(false); navigateTo('home'); }} className="text-slate-600 hover:text-teal-600 transition-colors font-medium">How It Works</a>
                            <a href="#features" onClick={() => { setIsMenuOpen(false); navigateTo('home'); }} className="text-slate-600 hover:text-teal-600 transition-colors font-medium">Features</a>
                           ...
                           <>
                            <button onClick={() => { navigateTo('dashboard'); setIsMenuOpen(false); }} className="text-slate-600 hover:text-teal-600">Dashboard</button>
                            <button onClick={() => { navigateTo('profile'); setIsMenuOpen(false); }} className="text-slate-600 hover:text-teal-600">Profile</button>
                            <button onClick={() => { onLogout(); setIsMenuOpen(false); }} className="bg-rose-500 text-white px-5 py-2 rounded-lg font-semibold w-1/2">
                                Logout
                            </button>
                           </>
                        )}
